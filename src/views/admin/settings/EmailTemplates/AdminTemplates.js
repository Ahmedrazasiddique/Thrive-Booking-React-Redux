...
class AdminTemplates extends React.Component {
 ...
 componentDidMount()
 {
     this.setState({adminEmailTemplateData:this.props.adminEmailTemplateData})
 }
  ...
  toggleCollapse = collapseID => {
    ...
    onEditorStateChange = editorState => {
        this.setState({
          editorState,
          adminEmailTemplateData:this.props.adminEmailTemplateData
        })
      }
    ...
    componentDidUpdate(prevProps,prevSate)
{
  ...
  render() {
    const { adminEmailTemplateData } = this.state
    const data=[];
    data.push(adminEmailTemplateData)  

    const { editorState } = this.state;
    const accordionShadowItems = data.map(adminEmailTemplateData => {
  ...
  <Loader isShowLoader={this.state.isShowLoader}></Loader>
            <Card className="collapse-border-item collapse-margin" className={classnames({
                "collapse-collapsed":
                  ...
                  this.state.collapseID === adminEmailTemplateData.id,
                "collapse-shown":
              ...
              <CardHeader  
              key={adminEmailTemplateData.id}
            >
                  <Col sm="6">
                <CardTitle className="lead collapse-title">
                {adminEmailTemplateData.subject}
...
{this.state.guestUserCheckOut? <Minus size={24}    onClick={() => this.toggleCollapse(adminEmailTemplateData.id)} ></Minus>
: <Plus size={24}    onClick={() => this.toggleCollapse(adminEmailTemplateData.id)} ></Plus>
}
</label>     
            </Col>
              </CardHeader>
              <Collapse
                isOpen={adminEmailTemplateData.id === this.state.collapseID}
            ...
            editorState={editorState.getCurrentContent().hasText()?editorState: EditorState.createWithContent(ContentState.createFromBlockArray(htmlToDraft(adminEmailTemplateData.message?adminEmailTemplateData.message:"<p>test</p>")))}
          ...
          /></CardBody>
            ...
            <Save className="mr-1" onClick={this.onSave(adminEmailTemplateData.id)} id={adminEmailTemplateData.id} size={23} />
...
export default connect(mapStateToProps, actionMethods)(AdminTemplates);