...
const UserType = (props) => {
    ...
    userRoles = roles.map((role, index) => {
      return (
        <Col
          ...
          >
            <Card className={classnames("fonticon-container", {
  ...
  if (!isEmpty(businesses) && Object.keys(businesses).length > 1) {
    ...
    userBusinesses = businesses.map((business, index) => {
      ...
      return (
        <Col xl="3" lg="3" sm="6" key={index} onClick={() => setUserBusiness(businessId)}>
  ...
  return (
    <Container>
      ...
      <Col md="12" className="text-right mt-2 mb-2">
        <Button.Ripple color="dark" className="mr-1" size="lg" onClick={onCloseHandler}>
          Cancel
        </Button.Ripple>
